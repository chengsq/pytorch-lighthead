...
class imagenet3d(imdb):
    def __init__(self, image_set, imagenet3d_path=None):
        ...
        self._imagenet3d_path = self._get_default_path(
        ) if imagenet3d_path is None else imagenet3d_path
    ...
    def _load_imagenet3d_annotation(self, index):
        ...
        if self._image_set == 'test' or self._image_set == 'test_1' or self._image_set == 'test_2':
            ...
            filename = os.path.join(
                self._imagenet3d_path, 'Labels', index + '.txt')
            lines = []
        ...
        viewpoints = np.zeros((num_objs, 3), dtype=np.float32)
        # azimuth, elevation, in-plane rotation
        viewpoints_flipped = np.zeros((num_objs, 3), dtype=np.float32)
        ...
        for ix, line in enumerate(lines):
            ...
            if len(words) == 8:
                ...
                viewpoints_flipped[ix, 2] = - \
                    viewpoints[ix, 2]  # in-plane rotation
            else:
        ...
        subindexes_flipped = np.zeros(
            (num_objs, self.num_classes), dtype=np.int32)
        viewindexes_azimuth = np.zeros(
            (num_objs, self.num_classes), dtype=np.float32)
        ...
        if cfg.IS_RPN:
            if cfg.IS_MULTISCALE:
                ...
                # compute overlap
                overlaps_grid = bbox_overlaps(
                    boxes_grid.astype(
                        np.float), boxes_all.astype(
                        np.float))
                ...
                if num_objs != 0:
                    ...
                    for k in xrange(1, self.num_classes):
                        fg_inds.extend(np.where((gt_classes_all == k) & (
                            max_overlaps >= cfg.TRAIN.FG_THRESH[k - 1]))[0])
                ...
                if num_objs != 0:
                    ...
                    fg_inds = []
                    for k in xrange(1, self.num_classes):
                        fg_inds.extend(np.where((gt_classes == k) & (
                            max_overlaps >= cfg.TRAIN.FG_THRESH[k - 1]))[0])
    ...
    def evaluate_detections_one_file(self, all_boxes, output_dir):
        ...
        for cls_ind, cls in enumerate(self.classes):
            ...
            # open results file
            filename = os.path.join(
                output_dir, 'detections_{}.txt'.format(cls))
            print 'Writing imagenet3d results to file ' + filename
            with open(filename, 'wt') as f:
                # for each image
                for im_ind, index in enumerate(self.image_index):
