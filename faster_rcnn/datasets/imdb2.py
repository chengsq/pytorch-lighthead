...
class imdb(object):
    ...
    def create_roidb_from_box_list(self, box_list, gt_roidb):
        ...
        for i in xrange(self.num_images):
            ...
            subindexes = np.zeros(
                (num_boxes, self.num_classes), dtype=np.int32)
            subindexes_flipped = np.zeros(
                (num_boxes, self.num_classes), dtype=np.int32)
            if cfg.TRAIN.VIEWPOINT or cfg.TEST.VIEWPOINT:
                viewindexes_azimuth = np.zeros(
                    (num_boxes, self.num_classes), dtype=np.float32)
                viewindexes_azimuth_flipped = np.zeros(
                    (num_boxes, self.num_classes), dtype=np.float32)
                viewindexes_elevation = np.zeros(
                    (num_boxes, self.num_classes), dtype=np.float32)
                viewindexes_elevation_flipped = np.zeros(
                    (num_boxes, self.num_classes), dtype=np.float32)
                viewindexes_rotation = np.zeros(
                    (num_boxes, self.num_classes), dtype=np.float32)
            ...
            if gt_roidb is not None:
                ...
                if gt_boxes.shape[0] != 0 and num_boxes != 0:
                    ...
                    subindexes[I, gt_classes[argmaxes[I]]
                               ] = gt_subclasses[argmaxes[I]]
                    subindexes_flipped[I, gt_classes[argmaxes[I]]
                                       ] = gt_subclasses_flipped[argmaxes[I]]
                    if cfg.TRAIN.VIEWPOINT or cfg.TEST.VIEWPOINT:
                        viewindexes_azimuth[I, gt_classes[argmaxes[I]]
                                            ] = gt_viewpoints[argmaxes[I], 0]
                        viewindexes_azimuth_flipped[I, gt_classes[argmaxes[I]]
                                                    ] = gt_viewpoints_flipped[argmaxes[I], 0]
                        viewindexes_elevation[I, gt_classes[argmaxes[I]]
                                              ] = gt_viewpoints[argmaxes[I], 1]
                        viewindexes_elevation_flipped[I, gt_classes[argmaxes[I]]
                                                      ] = gt_viewpoints_flipped[argmaxes[I], 1]
                        viewindexes_rotation[I, gt_classes[argmaxes[I]]
                                             ] = gt_viewpoints[argmaxes[I], 2]
                        viewindexes_rotation_flipped[I, gt_classes[argmaxes[I]]
                                                     ] = gt_viewpoints_flipped[argmaxes[I], 2]
        ...
        for i in xrange(len(a)):
            ...
            if cfg.TRAIN.VIEWPOINT or cfg.TEST.VIEWPOINT:
                a[i]['gt_viewpoints'] = np.vstack((a[i]['gt_viewpoints'],
                                                   b[i]['gt_viewpoints']))
                a[i]['gt_viewpoints_flipped'] = np.vstack(
                    (a[i]['gt_viewpoints_flipped'], b[i]['gt_viewpoints_flipped']))
                a[i]['gt_viewindexes_azimuth'] = scipy.sparse.vstack(
                    (a[i]['gt_viewindexes_azimuth'], b[i]['gt_viewindexes_azimuth']))
                a[i]['gt_viewindexes_azimuth_flipped'] = scipy.sparse.vstack(
                    (a[i]['gt_viewindexes_azimuth_flipped'], b[i]['gt_viewindexes_azimuth_flipped']))
                a[i]['gt_viewindexes_elevation'] = scipy.sparse.vstack(
                    (a[i]['gt_viewindexes_elevation'], b[i]['gt_viewindexes_elevation']))
                a[i]['gt_viewindexes_elevation_flipped'] = scipy.sparse.vstack(
                    (a[i]['gt_viewindexes_elevation_flipped'], b[i]['gt_viewindexes_elevation_flipped']))
                a[i]['gt_viewindexes_rotation'] = scipy.sparse.vstack(
                    (a[i]['gt_viewindexes_rotation'], b[i]['gt_viewindexes_rotation']))
                a[i]['gt_viewindexes_rotation_flipped'] = scipy.sparse.vstack(
                    (a[i]['gt_viewindexes_rotation_flipped'], b[i]['gt_viewindexes_rotation_flipped']))

            a[i]['gt_subclasses'] = np.hstack((a[i]['gt_subclasses'],
                                               b[i]['gt_subclasses']))
            a[i]['gt_subclasses_flipped'] = np.hstack(
                (a[i]['gt_subclasses_flipped'], b[i]['gt_subclasses_flipped']))
            a[i]['gt_overlaps'] = scipy.sparse.vstack([a[i]['gt_overlaps'],
                                                       b[i]['gt_overlaps']])
            a[i]['gt_subindexes'] = scipy.sparse.vstack(
                (a[i]['gt_subindexes'], b[i]['gt_subindexes']))
            a[i]['gt_subindexes_flipped'] = scipy.sparse.vstack(
                (a[i]['gt_subindexes_flipped'], b[i]['gt_subindexes_flipped']))
        return a
