...
class kitti(imdb):
    ...
    def image_path_from_index(self, index):
        ...
        image_path = os.path.join(
            self._data_path, prefix, index + self._image_ext)
    ...
    def _load_image_set_index(self):
        ...
        image_set_file = os.path.join(
            self._kitti_path, self._image_set + '.txt')
    ...
    def _load_selective_search_roidb(self, gt_roidb):
        ...
        if os.path.exists(cache_file):
            ...
            for index in self.image_index:
                ...
                raw_data = np.loadtxt(filename, dtype=float)
                box_list.append(
                    raw_data[:min(self.config['top_k'], raw_data.shape[0]), 1:])
            ...
            for index in self.image_index:
                ...
                raw_data = np.loadtxt(
                    filename, usecols=(
                        2, 3, 4, 5), dtype=float)
                box_list.append(
                    raw_data[:min(self.config['top_k'], raw_data.shape[0]), :])
