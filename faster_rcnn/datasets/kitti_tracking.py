...
class kitti_tracking(imdb):
    def __init__(self, image_set, seq_name, kitti_tracking_path=None):
        ...
        self._kitti_tracking_path = self._get_default_path(
        ) if kitti_tracking_path is None else kitti_tracking_path
        self._data_path = os.path.join(
            self._kitti_tracking_path, image_set, 'image_02')
        ...
        assert os.path.exists(
            filename), 'Path does not exist: {}'.format(filename)
        ...
        assert os.path.exists(
            self._kitti_tracking_path), 'kitti_tracking path does not exist: {}'.format(
            self._kitti_tracking_path)
