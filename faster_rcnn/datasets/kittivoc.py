    ...
    def _remove_empty_samples(self):
        ...
        for i in range(len(self._image_index) - 1, -1, -1):
            ...
            filename = os.path.join(
                self._data_path, 'Annotations', index + '.xml')
            tree = ET.parse(filename)
            objs = tree.findall('object')
            non_diff_objs = [obj for obj in objs if int(obj.find(
                'difficult').text) == 0 and obj.find('name').text.lower().strip() != 'dontcare']
    ...
    def _load_pascal_annotation(self, index):
        ...
        for ix, obj in enumerate(objs):
            ...
            if class_name != 'dontcare':
                care_inds = np.append(
                    care_inds, np.asarray(
                        [ix], dtype=np.int32))
            if class_name == 'dontcare':
                dontcare_inds = np.append(
                    dontcare_inds, np.asarray(
                        [ix], dtype=np.int32))
