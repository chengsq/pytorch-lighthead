...
def proposal_target_layer(
    ...
    all_rois = np.vstack((all_rois, np.hstack(
        (zeros, np.vstack((gt_easyboxes[:, :-1], jittered_gt_boxes[:, :-1]))))))
...
def _sample_rois(
    ...
    if cfg.TRAIN.PRECLUDE_HARD_SAMPLES and gt_ishard is not None and gt_ishard.shape[0] > 0:
        ...
        if gt_hardboxes.shape[0] > 0:
            ...
            ignore_inds = np.append(ignore_inds, np.where(
                hard_max_overlaps >= cfg.TRAIN.FG_THRESH)[0])
            if DEBUG:
