...
class PSRoIPoolingFunction(Function):
    ...
    def forward(self, features, rois):
        ...
        output = torch.zeros(
            ...
            self.pooled_height,
            self.pooled_width)
        mappingchannel = torch.IntTensor(self.output_dim, self.pooled_height, self.pooled_width).zero_()
        output = output.to(self.device)
        mappingchannel = mappingchannel.to(self.device)
    ...
    def backward(self, grad_output):
        ...
        grad_input = torch.zeros(batch_size, num_channels, data_height, data_width).to(self.device)
