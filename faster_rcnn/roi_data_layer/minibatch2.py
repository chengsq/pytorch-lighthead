...
def get_minibatch(roidb, num_classes):
    ...
    if cfg.IS_MULTISCALE:
        ...
        random_scale_inds = npr.randint(0, high=len(
            cfg.TRAIN.SCALES_BASE), size=num_images)
    ...
    if cfg.TRAIN.HAS_RPN:
        ...
        for im_i in xrange(num_images):
            ...
            if cfg.IS_MULTISCALE:
                if cfg.IS_EXTRAPOLATING:
                    rois, levels = _project_im_rois_multiscale(
                        im_rois, cfg.TRAIN.SCALES)
                    batch_ind = im_i * len(cfg.TRAIN.SCALES) + levels
                else:
                    rois, levels = _project_im_rois_multiscale(
                        im_rois, cfg.TRAIN.SCALES_BASE)
        ...
        if cfg.TRAIN.BBOX_REG:
            ...
            blobs['bbox_outside_weights'] = np.array(
                bbox_inside_blob > 0).astype(np.float32)
...
def _sample_rois(roidb, fg_rois_per_image, rois_per_image, num_classes):
    ...
    for i in xrange(1, num_classes):
        fg_inds.extend(
            np.where(
                (labels == i) & (
                    overlaps >= cfg.TRAIN.FG_THRESH))[0])
    ...
    for i in xrange(1, num_classes):
        bg_inds.extend(
            np.where(
                (labels == i) & (
                    overlaps < cfg.TRAIN.BG_THRESH_HI) & (
    ...
    if len(bg_inds) < bg_rois_per_this_image:
        for i in xrange(1, num_classes):
            bg_inds.extend(
                np.where(
                    (labels == i) & (
                        overlaps < cfg.TRAIN.BG_THRESH_HI))[0])
...
def _get_viewpoint_estimation_labels(viewpoint_data, clss, num_classes):
    ...
    inds = np.where((clss > 0) & np.isfinite(viewpoint_data[:, 0]) & np.isfinite(
        viewpoint_data[:, 1]) & np.isfinite(viewpoint_data[:, 2]))[0]
    for ind in inds:
        cls = clss[ind]
        start = 3 * cls
        end = start + 3
