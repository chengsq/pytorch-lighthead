...
from faster_rcnn.light_head import LH_RCNN, RPN
...
# hyper-parameters
# ------------
# imdb_name options: voc_2007_trainval, voc_2012_trainval, voc_0712_trainval
# voc_0712_trainval DOES NOT work now.
...
rand_seed = 1024
_DEBUG = True
use_tensorboard = False    # True/False whether using visualization (TensorBoard)
...
# load net
net = LH_RCNN(classes=imdb.classes, debug=_DEBUG)
...
# device check for CPU/GPU running
...
for step in range(start_step, end_step + 1):
    ...
    # train_loss += loss.data[0] will be an error in PyTorch 0.5
        ...
        if _DEBUG:
            ...
            log_print(
                ...
                (net.rpn.cross_entropy.data.cpu().numpy(),
                 net.rpn.loss_box.data.cpu().numpy(),
                 net.cross_entropy.data.cpu().numpy(),
                 net.loss_box.data.cpu().numpy()))
        re_cnt = True
    ...
    if use_tensorboard and step % log_interval == 0:
        ...
        if _DEBUG:
            ...
            losses = {
                'rpn_cls': float(
                    net.rpn.cross_entropy.data.cpu().numpy()), 'rpn_box': float(
                    net.rpn.loss_box.data.cpu().numpy()), 'rcnn_cls': float(
                    net.cross_entropy.data.cpu().numpy()), 'rcnn_box': float(
                    net.loss_box.data.cpu().numpy())}
            exp.add_scalar_dict(losses, step=step)

    if (step % 10000 == 0) and step > 0:
        save_name = os.path.join(output_dir,
                                 'faster_rcnn_vgg16_{}.h5'.format(step))
